        ...
        public async Task InsertCourtNumberMatchGroup(int categoryId, int groupNumber, int courtNumber, CancellationToken cancellationToken)
        {
            ...
            foreach (var match in matchGroups)
            {
                ...
                if (!string.IsNullOrEmpty(firstUser.FCMToken))
                {
                    try
                    {
                        _logger.LogInformation("Enviando notificação para o usuário {UserId} do grupo {GroupNumber} sobre a  número {CourtNumber}.", firstUser.Id, groupNumber, courtNumber);
                        await _firebaseService.SendNotificationAsync(
                            firstUser.FCMToken,
                            "Chamada de Quadra",
